...
export default function MyTokens({ contract }) {
    ...
    // store user owned NFTs
    const [myTokens, setMyTokens] = useState(null)
    // for music player
    const audioRefs = useRef([]);
    const [isPlaying, setIsPlaying] = useState(null)
    // keep track which song was played, so that we know which one to pause
    ...
    const resellItem = async (item) => {
        // checking to ensure valid resell price & token ID
        if (resellPrice === "0" || item.itemId !== resellId || !resellPrice) {
            return
        }

        // user has to pay royalty fee
